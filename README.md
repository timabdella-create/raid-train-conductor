...
## 31. Faster Edits On Live Trains

- Editing an existing train no longer means clicking through all 6 wizard steps to reach the Save button. Organizers editing a train (not creating a new one) can now click any step in the progress bar to jump straight to it, and a "Save changes" button appears on every step — not just Review — so a quick change (swap the banner image, tweak the description) is a two-click save instead of a full re-walkthrough.
- The quick-save button always submits `action="draft"`, which is a no-op on status either way — only the explicit "Publish train" button on the Review step can move a draft train live, so a quick save from step 1 can never accidentally publish something the organizer isn't ready to publish.
- The schedule (date/time/slot length) used to lock the moment a train was published, full stop — even with zero applicants, you had to unpublish just to fix a typo'd start time. It now locks based on actual engagement instead: **`lib/trains/schedule-lock.ts`** (new) checks for any confirmed seller (`train_participants`) or pending application, and only locks the schedule if one exists. A published train nobody's applied to yet can have its schedule freely edited, in place, with no unpublish/republish round trip. The moment someone's confirmed or has an application in flight, it locks — same protection as before, just scoped to when it actually matters.
- This check runs both in the edit page (to show/hide the lock in the UI) and inside `updateTrain` itself (so a stale page can't submit a schedule change past a seller who confirmed in the meantime).

## 32. Banner Image Cropping Fix

- The train banner (behind the title on the public train page) renders as a full-width, short strip, so most uploaded images needed heavy horizontal cropping to fill it — and on very wide monitors the strip's aspect ratio got extreme enough that organizers were losing the important part of their image entirely.
- The banner's container is now capped at `max-w-[1600px]` (still edge-to-edge on typical screens, just no longer stretching to an absurd ratio on ultra-wide monitors) and slightly taller at large breakpoints (`h-56` mobile → `sm:h-72` → `lg:h-80`), both of which reduce how aggressively any given image needs to be cropped.
- **`supabase/migrations/0016_banner_image_position.sql`** (new, applied): adds `image_position` to `raid_trains` (`'top' | 'center' | 'bottom'`, default `'center'`). The wizard's Basic Details step shows a "Banner focal point" control once an image is uploaded — if the subject is getting cropped off the top or bottom, switching this nudges the crop's anchor instead of requiring the organizer to re-edit the file externally. Applied via `object-top`/`object-center`/`object-bottom` on both the main train page banner and the smaller train-card thumbnails on the seller's trains list.

## 33. Banner "Show Whole Image" Fit Mode

- The banner's rendered shape varies a lot by screen size — close to square on mobile (~1.35:1) but panoramic on desktop (~3.3:1) — so a single crop-to-fill treatment can't be right for both at once. A wide graphic with text running edge-to-edge (a poster-style banner, not a plain photo) will lose content on the sides on mobile even if it looks fine on desktop, and there's no image shape that avoids this under object-cover.
- **`supabase/migrations/0017_banner_image_fit.sql`** (new, applied): adds `image_fit` to `raid_trains` (`'cover' | 'contain'`, default `'cover'`, preserving existing behavior for every train that hasn't opted in).
...
- Organizers can now paste a Discord webhook URL into a train's Rules step (wizard step 5). Three things fire off it:
  1. **Live "slot claimed" post** — whenever a seller successfully signs up (`submitApplication` in `app/train/[slug]/apply/actions.ts`), a message posts immediately naming the seller (their Whatnot username, e.g. `@EddieBanks`, looked up from `seller_profiles` at the moment of signup) and the slot's local time (e.g. "8:50 PM"): "claimed" for open/invite_only trains that confirm instantly, or "pending your approval" for approval_required trains — either way, the slot has left the open pool.
  2. **Live "slot opened" post** — whenever a confirmed seller drops their slot (seller-initiated cancellation via `cancelParticipation` in `app/dashboard/seller/actions.ts`, or organizer removal via `removeSellerFromSlot` in `app/dashboard/organizer/trains/[trainId]/schedule/actions.ts`), a message posts immediately naming who dropped and which slot time they held, plus the current open-slot count and a link. `cancelParticipation` has to look up the seller's `train_participants` row *before* calling the `cancel_train_participation()` RPC, since that RPC deletes the row — it's the only chance to know which slot they were dropping.
  3. **Daily 8am ET roundup** — a new cron route, **`app/api/cron/discord-daily-summary/route.ts`** (see `vercel.json`, `0 12 * * *` UTC — approximates 8am America/New_York; drifts an hour across the DST changeover twice a year, same caveat as the weekly report), posts every published/live train's remaining open slot times, for every train with a webhook configured whose event date hasn't passed yet.
...
## 40. Fix cancel_train_participation Silently No-Oping On Seller Cancellation

- Found while testing the Discord drop notification: `cancel_train_participation()`'s `INSERT INTO seller_history` used a `CASE` expression built entirely of untyped string literals for the `attendance_status` enum column. In a raw SQL context (`INSERT ... VALUES`), Postgres resolves an all-string-literal `CASE` to `text`, and assigning that to an enum column fails — same root cause as migration 0021's fix for `submit_train_application`, just a different function.
- That exception rolled back the entire function: the slot never reopened, the application was never withdrawn, and the `train_participants` row was never deleted. But `cancelParticipation()` (`app/dashboard/seller/actions.ts`) never checked the RPC's `error`, so it went on to send the "you're cancelled" notification and post the Discord "slot opened" message regardless — sellers saw a successful-looking cancellation and Discord announced an open slot that was, in the database, still filled.
...
## 42. Friendly Error For Duplicate Active Applications

- `submit_train_application()` relied entirely on the `train_applications_active_unique` partial index (migration 0023) to reject a second application while one's already pending/approved on the same train. The raw Postgres constraint error was leaking straight through to the seller-facing form (`app/train/[slug]/apply/actions.ts` just does `return { error: error.message }`), so a seller who held a second slot on a train they were already confirmed/pending on saw `duplicate key value violates unique constraint "train_applications_active_unique"` instead of a real message.
- **`supabase/migrations/0027_friendly_duplicate_application_error.sql`** (new, applied): adds an explicit pre-check that raises "You already have an active application on this train. Check your applications, or cancel it first if you want a different slot." before the insert even runs. The unique index stays in place as a backstop.
- Confirmed live: this had actually happened — a stray `held` slot on Pen Battle was stuck from someone hitting this exact case. Released it, and verified the new error message fires correctly by reproducing the same scenario directly against the database.
