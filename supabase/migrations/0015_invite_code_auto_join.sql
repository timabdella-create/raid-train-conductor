-- ============================================================================
-- Raid Train Conductor — Invite-code auto-join
-- Wires up the invite_code column (already on raid_trains, already used to
-- gate page visibility for private trains) to actually do something for
-- "invite only" signup trains: a seller who applies with the correct code
-- is confirmed immediately, skipping organizer review entirely. Previously
-- submit_train_application() unconditionally rejected every application on
-- an invite_only train regardless of whether the caller had a code — this
-- was a dead end with no way to actually join.
--
-- invite_code generation is also broadened: it used to only get generated
-- for private-visibility trains. Now it's generated whenever a train is
-- private OR invite_only, since invite_only trains need a code to hand out
-- even if they're publicly visible/listed (visibility and signup_mode are
-- independent settings — one controls who can see the train, the other
-- controls who can join it).
-- ============================================================================

drop function if exists public.submit_train_application(uuid, text, text, jsonb);
